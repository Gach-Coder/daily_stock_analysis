...
- [新功能] 新增 `name_to_code_list` 服务，支持将含市场指示符、拼音/汉字混合、数字代码等模糊股票输入解析为候选代码列表（A股→港股→美股 排序，最多 5 条），通过 token 切分、拼音变体匹配、错别字容错和 AkShare 在线兜底等多级漏斗提升命中率，每次匹配后进行市场矛盾检测以过滤非法输入。
